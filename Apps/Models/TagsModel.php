...
class TagsModel extends CommonModel
{
    ...
    {
        if (is_int($tagid)) {
            $res = self::$db->where(['id' => $tagid])->select($this->table_name);
            $tagname = $res[0]['tag_name'];
        } else {
            $tagid_list = explode(',', $tagid);
            $tagname = [];
            foreach ($tagid_list as $single_tag) {
                $res = self::$db->where(['id' => intval($single_tag)])->select($this->table_name);
                $tagname[] = array(
                    'name' => $res[0]['tag_name'],
    ...
    {
        $result = self::$db->where(['tag_name' => $tagname])->select($this->table_name);
