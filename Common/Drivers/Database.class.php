...
class Database
{
    ...
    protected static $_instance = null;//静态属性,所有数据库实例共用,避免重复连接数据库
     ...
     */
    private function __construct(array $dbn)
    {
        if ((null === self::$_dbh)) {
            $this->_dbName = $dbn['DB_NAME'];
            $dsn = $dbn['DB_TYPE'] . ':host=' . $dbn['DB_SERVER'] . ';port=' . $dbn['DB_PORT'] . ';dbname=' . $dbn['DB_NAME'];
            $options = $dbn['DB_PCONNECT'] ? array(\PDO::ATTR_PERSISTENT => true) : array();
            try {
                $_dbh = new \PDO($dsn, $dbn['DB_USER'], $dbn['DB_PASSWORD'], $options);
                $_dbh->setAttribute(\PDO::ATTR_ERRMODE, \PDO::ERRMODE_EXCEPTION);  //设置如果sql语句执行错误则抛出异常，事务会自动回滚
                $_dbh->setAttribute(\PDO::ATTR_EMULATE_PREPARES, false); //禁用prepared statements的仿真效果(防SQL注入)
            } catch (\PDOException $e) {
                $errMsg = $e->getMessage();
                $this->logError($errMsg);
                die('Database connection failed: ' . $errMsg);
            }
            $_dbh->exec('SET NAMES ' . $dbn['DB_CODE']);
            self::$_dbh = $_dbh;
        }
    }

    public static function getSingleton(array $dbn)
    {
        //连接数据库
        if ((null === self::$_instance)) {
            self::$_instance = new self($dbn);
        }
